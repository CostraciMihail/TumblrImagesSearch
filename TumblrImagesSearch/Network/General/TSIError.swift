...
struct TSIError: Error {

    var code: Int
    var codeKey: TSIErrorKeys
    var domain: String
    var message: String?
    var userInfo: Any?

    init(code: Int,
         codeKey: TSIErrorKeys = .UNKNOWN_ERROR,
         domain: String = TSIAppContext.bundleID,
         message: String? = nil,
         userInfo: Any? = nil) {

        self.code = code
        self.codeKey = TSIErrorKeys.errorKey(from: code)
        self.domain = domain
        self.message = message ?? codeKey.localized()
    ...
    init(urlError: URLError) {
        ...
        self.codeKey = TSIErrorKeys.errorKey(from: urlError.errorCode)
        self.domain = ""
        self.message = message ?? codeKey.localized()
    ...
    init(nsError: NSError) {
        ...
        self.codeKey = TSIErrorKeys.errorKey(from: nsError.code)
    ...
    init(error: Error) {
        ...
        self.code = TSIErrorKeys.UNKNOWN_ERROR.rawValue
        self.codeKey = TSIErrorKeys.UNKNOWN_ERROR
        self.domain = TSIAppContext.bundleID
        self.message = error.localizedDescription.capitalized
        self.userInfo = error
    }

    static func toError(error: Error) -> TSIError {

        if let urlError = error as? URLError {
            return TSIError(urlError: urlError)

        } else if let _error = error as? TSIError {
            return _error
        } else {
            return TSIError(error: error)
        }
    }

    static func unknownError() -> TSIError {
        return TSIError(code: TSIErrorKeys.UNKNOWN_ERROR.rawValue)
    }
}
...
extension TSIErrorKeys {
    ...
    public static func errorKey(from code: Int) -> TSIErrorKeys {
    ...
    /// Return localized text for current error.
    public func localized() -> String {
        let key = "ERROR." + self.toString
        return localize(key)
    }
}
